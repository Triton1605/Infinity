...
class DataManager:
    ...
    def get_asset_folder_path(self, asset_type: str, symbol: str) -> str:
        """Get the folder path for an asset."""
        asset_dir = self.filepath_manager.get_asset_dir(asset_type)
        return os.path.join(asset_dir, symbol)
    
    def get_asset_data_file_path(self, asset_type: str, symbol: str) -> str:
        """Get the data.json file path for an asset."""
        asset_folder = self.get_asset_folder_path(asset_type, symbol)
        return os.path.join(asset_folder, "data.json")
    
    def get_asset_events_file_path(self, asset_type: str, symbol: str) -> str:
        """Get the events.json file path for an asset."""
        asset_folder = self.get_asset_folder_path(asset_type, symbol)
        return os.path.join(asset_folder, "events.json")
    ...
    def save_asset_data(self, asset_data: Dict) -> bool:
        """
        Save asset data to JSON file in asset's folder.
        ...
        try:
            ...
            # Save data.json
            file_path = self.get_asset_data_file_path(asset_type, symbol)
            with open(file_path, 'w') as f:
                json.dump(asset_data, f, indent=2)
            
            # Create empty events.json if it doesn't exist
            events_path = self.get_asset_events_file_path(asset_type, symbol)
    ...
    def asset_exists(self, symbol: str, asset_type: str) -> bool:
        """Check if an asset folder exists."""
        asset_folder = self.get_asset_folder_path(asset_type, symbol)
        return os.path.exists(asset_folder)
    
    def remove_asset_from_tracking(self, symbol: str, asset_type: str) -> bool:
        """
        Remove an asset from tracking and delete its entire folder (including events).
        ...
        try:
            ...
            # Delete the entire asset folder (including events)
            asset_folder = self.get_asset_folder_path(asset_type, symbol)
            if os.path.exists(asset_folder):
                shutil.rmtree(asset_folder)
                print(f"Deleted asset folder: {asset_folder}")
    ...
    def load_asset_events(self, symbol: str, asset_type: str) -> List[Dict]:
        """
        Load events for an asset from its events.json file.
    ...
    def save_asset_events(self, symbol: str, asset_type: str, events: List[Dict]) -> bool:
        """
        Save events for an asset to its events.json file.
